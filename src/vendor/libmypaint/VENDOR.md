...
and not appropriate defaults for a distributed R package. The vendored build
uses R's configured compiler flags, plus the package-local `PKG_CPPFLAGS`,
`PKG_LIBS`, and `OBJECTS` needed to compile and link the vendored sources.
