...
And close the device with `dev.off()` to print your plot to the output file:
...
Here are some brushes that I've found good to use, i.e. neither
too crazy nor too similar to standard R:
...
```{r, mypaint=TRUE, fig.keep="none", fig.height=5, fig.width=5}
...
There is one glitch with the mypaint_device: as you may have spotted, borders
...
```{r, mypaint=TRUE, fig.keep="none", fig.width=5, fig.height=5}
...
You can also use the special geoms `geom_mypaint_bar()` and `geom_mypaint_col()`. These have two advantages over `mypaint_wrap()`:

* You can use special fill patterns, like `zigzag()` and `jumble()`.
* They affect the legend key as well as the actual plot element. (Compare the keys above and below.)
