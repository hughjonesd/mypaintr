---
title: "ggplot examples with mypaintr"
...
creating high-quality graphs. To experiment, I recreated some graphs from the
ggplot2 examples, using mypaintr to communicate information in a fun, stylish
and clear way.
...
```{r geom-contour-filled, mypaint=TRUE, fig.keep='none'}
...
```{r geom-smooth, mypaint=TRUE, fig.keep='none'}
...
you in time. But to do that effectively, you really need a single path, to
avoid confusing the eye when "earlier" lines overwrite "later" lines.

```{r geom-path, mypaint=TRUE, fig.keep='none'}
...
```{r geom-hex, mypaint=TRUE, fig.keep='none'}
...
```{r geom-dotplot, mypaint=TRUE, fig.keep='none'}
...
Here the sewing dashes convey uncertainty nicely. That's probably sensible,
given the small number of observations.

I'm not sure if I love or hate this colour scheme. It is vaguely 
[Lady Penelope](https://thunderbirds.fandom.com/wiki/Lady_Penelope_Creighton-Ward).

```{r geom-violin, mypaint=TRUE, fig.keep='none'}
